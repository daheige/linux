```
#ubuntu16.04安装lnmp以及mongodb,redis安装
#安装nginx 会自动创建www-data用户
...
#nginx 虚拟主机配置参考 https://github.com/daheige/linux/blob/master/nginx_vhost.md

sudo vim /var/www/html/p.php
加入如下内容
<?php
phpinfo();
...
#访问localhost/p.php 可看到phpinfo的页面
...
sudo mv phpMyAdmin-4.8.0-all-languages phpmyadmin
cd phpmyadmin
sudo cp config.sample.inc.php config.inc.php
sudo vim config.inc.php #在31行修改localhost为127.0.0.1 $cfg['Servers'][$i]['host'] = '127.0.0.1';

#访问http://localhost/phpmyadmin/ 可以通过root和密码登录了
```
